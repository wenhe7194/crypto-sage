...
def LFSRWithWorkMode():
    ...
    执行LFSR工作模式。
    """
    s.append((2 ** 15 * s[15] + 2 ** 17 * s[13] + 2 ** 21 * s[10] +
                       2 ** 20 * s[4] + (1 + 2 ** 8) * s[0]) % (2 ** 31 - 1))
    if s[16] == 0:
        s[16] = 2 ** 31 - 1
    s.pop(0)
...
def BitReconstruction():
    ...
    执行比特重构，生成重构的比特位。
    """
    X[0] = combine_high_low_bits(s[15], s[14])
    X[1] = combine_low_high_bits(s[11], s[9])
    X[2] = combine_low_high_bits(s[7], s[5])
    X[3] = combine_low_high_bits(s[2], s[0])
...
def F(x0, x1, x2):
    ...
    """
    global R1, R2
    W = mod32_add(x0 ^^ R1, R2)
    w1 = mod32_add(R1, x1)
    w2 = R2 ^^ x2
    R1 = S(l1(((w1 << 16) | (w2 >> 16)) & 0xffffffff))
    R2 = S(l2(((w2 << 16) | (w1 >> 16)) & 0xffffffff))
    return W
