...
class SHA224or256:
    ...
    def _compress(self, chunk: bytes):
        ...
        for i in range(64):
            ...
            ch = _band(e,f) ^^ _band(_bnot(e,32), g)
            t1 = _u32(h+S1+ch+Integer(self._K32[i])+W[i])
            S0 = _rotr(a,2,32) ^^ _rotr(a,13,32) ^^ _rotr(a,22,32)
            maj = _band(a,b) ^^ _band(a,c) ^^ _band(b,c)
            t2 = _u32(S0+maj)
...
class SHA384or512:
    __slots__ = ("_H","_out_words")
    _IV_512 = [0x6A09E667F3BCC908,0xBB67AE8584CAA73B,0x3C6EF372FE94F82B,0xA54FF53A5F1D36F1,
               0x510E527FADE682D1,0x9B05688C2B3E6C1F,0x1F83D9ABFB41BD6B,0x5BE0CD19137E2179]
    _IV_384 = [0xCBBB9D5DC1059ED8,0x629A292A367CD507,0x9159015A3070DD17,0x152FECD8F70E5939,
    ...
    def _compress(self,chunk:bytes):
        ...
        for i in range(80):
            ...
            ch=_band(e,f) ^^ _band(_bnot(e,64),g)
            t1=_u64(h+S1+ch+Integer(self._K64[i])+W[i])
            S0=_rotr(a,28,64) ^^ _rotr(a,34,64) ^^ _rotr(a,39,64)
            maj=_band(a,b) ^^ _band(a,c) ^^ _band(b,c)
            t2=_u64(S0+maj)
            h,g,f,e,d,c,b,a=g,f,e,_u64(d+t1),c,b,a,_u64(t1+t2)
